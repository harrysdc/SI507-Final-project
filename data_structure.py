...
class Node: 
    def __init__(self, Y: list, X: pd.DataFrame, min_samples_split=None, max_depth=None, depth=None, node_type=None, rule=None):
        '''
        init a node for a decision tree
        '''
        self.Y = Y 
        self.X = X
    ...
    def GINI_impurity(y1_count, y2_count):
        """
        compute GINI impurity given # observations of a binary class 
        """
        if y1_count is None:
            y1_count = 0
    ...
    def best_split(self):
        '''
        calculate the best split for a decision tree given input X and target Y
        '''
        df = self.X.copy()
        df['Y'] = self.Y
        ...
        for feature in self.features:
            ...
            xmeans = self.get_moving_avg(Xdf[feature].unique(), 2)
