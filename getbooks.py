...
class Book:
    def __init__(self, json=None):
        '''
        read json and store all relevant info of a book
        '''
        if json is not None:
    ...
    def getTrainData(self, user_short, user_category, user_recent):
        '''
        convert str attribute to int for training
        '''
...
def getBooks(title):
    '''
    use requests to get books with keyword 'title'
    '''
...
def predict_decision_tree(clf, test_x, test_info, features = ['release_year', 'category', 'pageCnt']):
    '''
    predict test_y using classifier
    '''
...
def getLibrary(user_short, user_category, user_recent, mode='train'):
    '''
    get train/test set, each about 300 books
    '''
...
def write_html(title_list, author_list, category_list, link_list, year_list, preview_list, DISPLAY_TREE='y'):
    '''
    use library dominate to write a html file
    '''
    import glob
...
def display_in_webpage():
    '''
    use Flask to display html file
    '''
